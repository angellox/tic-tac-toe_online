...
pygame.display.set_caption('Juego Multijugador: Tic - Tac - Toe: Jugador 1')
...
def waiting_for_connection():
    ...
    conn, addr = server.accept() #HANDSHAKE, ESPERAMOS CONEXIÓN.
...
while running:
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and connection_established:
            if pygame.mouse.get_pressed()[0]: #CLIC EN EL MOUSE IZQUIERDO (1,0,0) POSICION [0]
                if turn and not grid.game_over:
                    ...
                    #send_data = '{}-{}-{}-{}'.format(cellX, cellY, "Vas", playing).encode() #CONVERTIR A STRING.
                    conn.send(send_data) #ENVÍO DE DATOS.
                    turn = False
    ...
    if grid.game_over:
        ...
        if time_sum < 1500:
            ...
            if grid.string_player == 'X':
                winner = font_sec.render('X GANÓ', True, (79, 214, 126))

            elif grid.string_player == 'O':
